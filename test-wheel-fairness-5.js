// Test script to verify wheel fairness with 5 names
// Simulates the same logic as the SpinningWheel component

function simulateWheel() {
  // Setup the wheel with 5 names + 2 RESPIN tiles
...
function runSimulation(numSpins = 1000000) {
  ...
  console.log('\n=== WHEEL FAIRNESS TEST RESULTS (5 NAMES) ===');
  ...
  console.log(`Critical value (95% confidence): ~12.59`);
  console.log(`Fair wheel? ${chiSquare < 12.59 ? 'YES ✓' : 'NO ✗'} (chi-square < critical value)`);

  return { results, maxDeviation, avgDeviation, respinDeviation: Math.abs(actualRespinPercentage - expectedRespinProbability) };
}

// Run the simulation
console.log('Running fairness test with 5 names and 1,000,000 spins...');
